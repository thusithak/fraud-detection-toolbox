...
! function(a) {
    ...
    h.DEFAULTS = {
        ...
        var c, d, e = this,
            ...
            g = 0,
            h = 0;
        this.$toolbar = this.$container.find(".fixed-table-toolbar").html(""), "string" == typeof this.options.toolbar && a('<div class="bars pull-left"></div>').appendTo(this.$toolbar).append(a(this.options.toolbar)), f = ['<div class="columns columns-' + this.options.toolbarAlign + " btn-group pull-" + this.options.toolbarAlign + '">'], this.options.showRefresh && f.push(b('<button class="btn btn-default" type="button" name="refresh" title="%s">', this.options.formatRefresh()), '<i class="glyphicon glyphicon-refresh icon-refresh"></i>', "</button>"), this.options.showFilter && f.push(b('<button class="btn btn-default" type="button" name="filter" title="%s">', this.options.formatFilter()), '<i class="glyphicon glyphicon-filter icon-filter"></i>', "</button>"), this.options.showToggle && f.push(b('<button class="btn btn-default" type="button" name="toggle" title="%s">', this.options.formatToggle()), '<i class="glyphicon glyphicon glyphicon-list-alt icon-list-alt"></i>', "</button>"), this.options.showColumns && (f.push(b('<div class="keep-open btn-group" title="%s">', this.options.formatColumns()), '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">', '<i class="glyphicon glyphicon-th icon-th"></i>', ' <span class="caret"></span>', "</button>", '<ul class="dropdown-menu" role="menu">'), a.each(this.options.columns, function(a, c) {
        ...
        for (var j = this.pageFrom - 1; j < this.pageTo; j++) {
            ...
            if (l = f(this.options, this.options.rowStyle, [k, j], l), l && l.css)
                for (var p in l.css) m.push(p + ": " + l.css[p]);
            if (n = f(this.options, this.options.rowAttributes, [k, j], n))
                for (var p in n) o.push(b('%s="%s"', p, g(n[p])));
        ...
        if (b.hasOwnProperty("field") && b.hasOwnProperty("values")) {
            for (c = e - 1; c >= 0; c--) {
                if (d = this.options.data[c], !d.hasOwnProperty(b.field)) return; - 1 !== a.inArray(d[b.field], b.values) && this.options.data.splice(c, 1)
            }
